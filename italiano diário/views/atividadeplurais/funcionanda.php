                        ...
                        <div id="collapse1_17" class="panel-collapse noScroll collapse" role="tabpanel" aria-labelledby="headingOne" data-parent="#accordion" data-bs-parent="#bootstrap-accordion_17">
                            <div class="panel-body">
                                <div class="container mt-4">
                                        ...
                                        <table class="table table-bordered table-striped">
                                            <thead class="table-light">
                                                <tr>
                                                    ...
                                                    <?php if ($isAdmin): ?> <!-- Exibir Ações apenas para admin -->
                                            ...
                                            <tbody id="activityBody">
                                                ...
                                                <tr>
                                                    ...
                                                    <td data-id="<?php echo $word['id']; ?>"><?php echo htmlspecialchars($word['singular']); ?></td>
                                    ...
                                    <?php if ($isAdmin): ?> <!-- Exibir o formulário de adicionar palavras apenas para admin -->
                                    <!-- Formulário para adicionar novas palavras -->
                                    <div class="container mt-4">
                                        <h4>Adicione uma nova palavra</h4>
                                        <div id="addWordsForm">
...
function salvarPalavras() {
    ...
    rows.forEach(row => {
        ...
        const wordId = row.cells[1].getAttribute('data-id'); // Verifica se a palavra já tem um ID

        if (!wordId && singular) { // Se não houver ID, é uma palavra nova
            words.push({ singular: singular });
        }
    });

    if (words.length > 0) { // Somente enviar se houver novas palavras
        const xhr = new XMLHttpRequest();
        xhr.open("POST", "salvar_palavras.php", true);
        xhr.setRequestHeader("Content-Type", "application/json");
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 200) {
                alert('Palavras novas salvas com sucesso!');
                location.reload(); // Atualizar a página para exibir as palavras salvas
            }
        };
        xhr.send(JSON.stringify(words));
    } else {
        alert('Não há novas palavras para salvar.');
    }
}
